...
function addLocalVideo(role) {
    if (role == 'Professeur') {
        Twilio.Video.createLocalVideoTrack().then(track => {
            let video = document.getElementById('local').firstChild;
            let video_professeur = document.getElementById('video_2');
            let trackElement = track.attach();
            trackElement.addEventListener('click', () => {
                zoomTrack(trackElement);
            });
            video_professeur.appendChild(trackElement);
        });
    } else {
        Twilio.Video.createLocalVideoTrack().then(track => {
            let video = document.getElementById('local').firstChild;
            let video_professeur = document.getElementById('video_1');
            let trackElement = track.attach();
            trackElement.addEventListener('click', () => {
                zoomTrack(trackElement);
            });
